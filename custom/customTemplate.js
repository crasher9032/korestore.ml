(function($){
	...
	customTemplate.prototype.renderMessage = function (msgData) {
		...
		if (msgData.message[0] && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.template_type == "dropdown_template") {
		...
		else if(msgData.message[0] && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.template_type == "widgetPedido"){
			try {
				$(".chatInputBox").hide();
				messageHtml = $(this.getChatTemplate("widgetPedido")).tmpl({
					'helpers': this.helpers,
					'extension': this.extension
				});
				this.bindEvents(messageHtml);
			...
			try {
				//var idCliente = msgData.message[0].component.payload.id;
				var idCliente = 1;
				$(".chatInputBox").hide();
		...
		else if(msgData.message[0] && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.template_type == "widgetVerificacion"){
			try {
				messageHtml = $(this.getChatTemplate("widgetVerificacion")).tmpl({
					'helpers': this.helpers,
					'extension': this.extension
				});
				//var idCliente = msgData.message[0].component.payload.id;
				...
				// caja.hide();
				let url = 'https://61532422c465200017d1a9f6.mockapi.io/Pedidos/' + idCliente;
				let datos = {};
				function continuar(mensaje){
					caja.show();
					caja.text("Continuar");
					chatInitialize.sendMessage(caja, mensaje);
				}
				async function call(){
					await fetch(url)
	...
	*/
	...
	customTemplate.prototype.getChatTemplate = function (tempType) {
		...
		*/
		var widgetVerificacion = `
		<div>
			<p>Selecciona tus productos y da click en finalizar pedido</p>
		</div>
		`;
		var widgetPedido = `
		<script type="text/x-jqury-tmpl">
			<div class="TavoComponent">
				<iframe src="https://pruebas-a4eae.web.app/cart/1" width="100%" height="300px"></iframe>
			</div>
		</script>
		`;
		var widgetPago = `
...
print(JSON.stringify(message)); */
	...
	{{/if}} \
		...
		if (tempType === "dropdown_template") {
			...
			return listWidget;
		}else if(tempType === "widgetPedido"){
			return widgetPedido;
		} else if(tempType === "widgetPago"){
			return widgetPago;
		}else if(tempType === "widgetVerificacion"){
			return widgetVerificacion;
		} else {
			return "";
		}
		return "";
...
//Below method is for template specific events
	...
	customTemplate.prototype.bindEvents = function (messageHtml) {
		...
		helpers=this.helpers;
		//funciones vue
		// $(messageHtml).find("#app").ready(function () {
		// 	new Vue({
		// 		el: "#app",
		// 		vuetify: new Vuetify(),
		// 		data: () => ({
		...
		// 		  async submit() {
		// 			//pagado: true
		// 			//datos de pago: data form
		// 			let body = {
		// 			  datos_pago: {
		// 				pagado: true,
		// 				datos_tarjeta: this.payment,
		// 			  },
		// 			};
		// 			let res = await axios.put(
		// 			  "https://61532422c465200017d1a9f6.mockapi.io/Pedidos/" + 1,
		// 			  body
		// 			);
		// 			console.log(res);
		// 			this.dialog = true;
		...
		var url = 'https://61532422c465200017d1a9f6.mockapi.io/Pedidos/' + 1;
		...
		async function submitAPI(){
			await fetch(url, {
				...
				body: JSON.stringify({
					datos_pago:{
						pagado: true,
						numtarjeta: $('.card-number-input').val(),
		...
		function sendEmail(){
			var emailDetails = {
				...
				template_id: 'template_ts59san',
				user_id: 'fsl1JfSxj_n2GNtyD',
				template_params: {
					polizaNbr: 1414141,
					vencimientoFecha: '07/2026',
					toMail: $('#email-input').val(),
					linkPoliza: "https://bouffant-laughs.000webhostapp.com/Documents/Poliza-qualitas.pdf"
				}
			};
